	...
	<view class="content">
		...
		<view class="comdisk-view">
			<uni-collapse accordion="true">
				...
				<uni-collapse-item v-if="filedata.role == 'admin'" title="音乐会员管理" :show-animation="true" thumb="../../static/img/admin.png">
					<view style="padding: 0rpx 30rpx 20rpx 30rpx;"><musiclogin></musiclogin></view>
				</uni-collapse-item>
			</uni-collapse>
		</view>
		<uni-popup ref="popuptip" :mask-click="false">
			<view class="uni-tip">
				...
				<view v-show="type == 'delete'" class="uni-tip-content">
					确定从公共云盘删除文件
					<br />
					{{ checkitem }}
				</view>
				<view v-show="type == 'move'" class="uni-tip-content">
					确定移动该文件至个人云盘么？同时会删除公共云盘内的该文件
					<br />
					{{ checkitem }}
				</view>
				<view v-show="type == 'copy'" class="uni-tip-content">
					确定复制该文件至个人云盘么？不会删除公共云盘内的该文件
					<br />
					{{ checkitem }}
				</view>
		...
		<!-- <outPut :outPutid="'output2'" class="output-view"></outPut> -->
	</view>
</template>
...
var Socket;
export default {
	components: { uniPopup, uniCollapse, uniCollapseItem, uniNoticeBar, uniList, uniListItem, uniSwipeAction, uniSwipeActionItem, musiclogin,lFile },
	data() {
		return {
			options: [
				{
		...
		if (val.index == 0) {
			if(this.uploadStatus){
				...
				this.$refs.lFile.upload({
					...
					url: api.upLoad, //测试地址，记得更换
					name: 'file',
					formData:{token:this.token}
				});
			}else{
				plus.nativeUI.toast('有正在进行的任务');
			}
		}
	},
	// onBackPress(options) {
	...
	methods: {
		...
		playIt(item, type) {
			if (type == 'music') {
				...
				uni.navigateTo({
					url: '../info/index?item=' + item + '&type=pubdoc',
					animationType: 'fade-in',
					animationDuration: 200
				});
			}
		...
		copyIt(item) {
			...
			uni.showLoading({ mask: true, title: '复制中...' });
			disk.copyFile(data)
				.then(response => {
					uni.hideLoading()
					let [error, res] = response;
		...
		getList() {
			...
			uni.request({
				url: api.getComDisk, //仅为示例，并非真实接口地址。
				method: 'POST',
				data: data,
				header: api.header,
				success: res => {
					if (res.data.type == 'error') {
						...
						this.$store.commit('FileVal', this.filedata);
						if (this.filedata.role == 'admin') {
							this.options.push({
								text: '删除',
								style: {
									backgroundColor: 'red'
